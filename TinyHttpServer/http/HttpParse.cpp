...
HttpRequestParser::parse_requestline(char *line, PARSE_STATE &parse_state, HttpRequest &request) {
    char *url = strpbrk(line, " \t");//这个函数是找第一个\t，包含的字符数目
    if (!url) {
        return BAD_REQUEST;
    }
    //反斜杠t就是GET和url的分界线
    ...
    *url++ = '\0';
    ...
    char *method = line;
    //\0分割后，line就是前半部分方法即GET等等，赋给了method
    ...
    url += strspn(url, " \t");//该函数返回 url 中第一个不在字符串“ \t” 中出现的字符下标。
    //此时GET "\0" (url起始点)url "\t"version
    char *version = strpbrk(url, " \t");//分割url和version
    if (!version) {
        return BAD_REQUEST;
    }
    *version++ = '\0';
    //此时GET \0 (url起始点)url "\0"version
    version += strspn(version, " \t");

    // HTTP/1.1 后面可能还存在空白字符
    if (strncasecmp("HTTP/1.1", version, 8) == 0) {//比较两个字符串前8个字符，忽略大小写
